...
void checker(bicycle* mas, int i, int k)
{
	...
	if (k == 2)
	{
		...
		printf("Введите цвет %d велосипеда:", i + 1);
		while (scanf_s("%s", mas[i].colour, 1024) != 1 || (mas[i].colour != NULL && strlen(mas[i].colour) < 3))
		{
...
void sort_1_field(bicycle* mas, const int* n)
{
...
int variants(int ch_1, int flag, const bicycle* mas)
{
	switch (ch_1)
	{
	case 1:
...
void sort_2_field(bicycle* mas, const int* n)
{
